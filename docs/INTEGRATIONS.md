...
프론트에서는 **`IntegrationStrip`** (`components/integration-strip.tsx`)가 홈·Market·News·Signal·Global Data·Research·Scan·Membership·내 정보(`/me`)·Exchange·시스템 트레이딩·관리자(`/admin`)·폼 미리보기(`/preview/auth`) 등 페이지 상단에서 동일 필드를 Crypto·KOSPI·US·News 칩으로 표시합니다. 메타는 루트 **`PlatformMetaProvider`** 가 `GET /api/platform/meta`를 한 번만 호출해 공유합니다. 타입은 **`lib/platform-meta-types.ts`**, 훅은 **`usePlatformMeta()`** / 연동 칩만 **`usePlatformIntegrations()`**.
